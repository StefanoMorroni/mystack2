...
ALTER TABLE geostore_test.gs_usergroup_members 
...
CREATE INDEX idx_attribute_name 
  ON geostore_test.gs_attribute (name); 

CREATE INDEX idx_attribute_resource 
  ON geostore_test.gs_attribute (resource_id); 

CREATE INDEX idx_attribute_text 
  ON geostore_test.gs_attribute (attribute_text); 

CREATE INDEX idx_attribute_type 
  ON geostore_test.gs_attribute (attribute_type); 

CREATE INDEX idx_attribute_date 
  ON geostore_test.gs_attribute (attribute_date); 

CREATE INDEX idx_attribute_number 
  ON geostore_test.gs_attribute (attribute_number); 

ALTER TABLE geostore_test.gs_attribute 
  ADD CONSTRAINT fk_attribute_resource FOREIGN KEY (resource_id) REFERENCES 
  gs_resource; 

CREATE INDEX idx_category_type 
  ON geostore_test.gs_category (name); 

CREATE INDEX idx_resource_name 
  ON geostore_test.gs_resource (name); 

CREATE INDEX idx_resource_description 
  ON geostore_test.gs_resource (description); 

CREATE INDEX idx_resource_metadata 
  ON geostore_test.gs_resource (metadata); 

CREATE INDEX idx_resource_update 
  ON geostore_test.gs_resource (lastupdate); 

CREATE INDEX idx_resource_creation 
  ON geostore_test.gs_resource (creation); 

CREATE INDEX idx_resource_category 
  ON geostore_test.gs_resource (category_id); 

ALTER TABLE geostore_test.gs_resource 
  ADD CONSTRAINT fk_resource_category FOREIGN KEY (category_id) REFERENCES 
  gs_category; 

CREATE INDEX idx_security_resource 
  ON geostore_test.gs_security (resource_id); 

CREATE INDEX idx_security_user 
  ON geostore_test.gs_security (user_id); 

CREATE INDEX idx_security_group 
  ON geostore_test.gs_security (group_id); 

CREATE INDEX idx_security_write 
  ON geostore_test.gs_security (canwrite); 

CREATE INDEX idx_security_read 
  ON geostore_test.gs_security (canread); 

CREATE INDEX idx_security_username 
  ON geostore_test.gs_security (username); 

CREATE INDEX idx_security_groupname 
  ON geostore_test.gs_security (groupname); 

ALTER TABLE geostore_test.gs_security 
...
ALTER TABLE geostore_test.gs_stored_data 
  ADD CONSTRAINT fk_data_resource FOREIGN KEY (resource_id) REFERENCES 
  gs_resource; 

CREATE INDEX idx_user_group 
  ON geostore_test.gs_user (group_id); 

CREATE INDEX idx_user_password 
  ON geostore_test.gs_user (user_password); 

CREATE INDEX idx_user_name 
  ON geostore_test.gs_user (name); 

CREATE INDEX idx_user_role 
  ON geostore_test.gs_user (user_role); 

ALTER TABLE geostore_test.gs_user 
  ADD CONSTRAINT fk_user_ugroup FOREIGN KEY (group_id) REFERENCES gs_usergroup; 

CREATE INDEX idx_user_attribute_name 
  ON geostore_test.gs_user_attribute (name); 

CREATE INDEX idx_user_attribute_text 
  ON geostore_test.gs_user_attribute (string); 

CREATE INDEX idx_attribute_user 
  ON geostore_test.gs_user_attribute (user_id); 

ALTER TABLE geostore_test.gs_user_attribute 
  ADD CONSTRAINT fk_uattrib_user FOREIGN KEY (user_id) REFERENCES gs_user; 

CREATE INDEX idx_usergroup_name 
  ON geostore_test.gs_usergroup (groupname); 
